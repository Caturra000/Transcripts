...
最后一步，我们如何实际解决这个问题？一个方法是切换到不同的调度技术。我们将要看的第一个叫做**线程化分派 (Threading)**。它叫 threading，但它和线程（threads）没有任何关系。这只是它的叫法。是的，不，就是这样，但它使用了相同的比喻，threads，所以它被称为 threading，但它与多线程无关。
...
你们都熟悉常规的 `goto`。你有一个语句的标签，然后你可以 `goto` 那个标签。如果你想写出可读性高的代码，它真的很好用。
